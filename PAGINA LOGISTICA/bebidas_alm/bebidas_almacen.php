...
<head>
    ...
    <title>Logystic | Almacen De Bebidas</title>
</head>
<body class="bg-dark">
  ...
  <div class="users-form">
        <h1>Registro</h1>
        <form action="bebidas_registro.php" method="POST">
            <div class="input-group mb-3">
              <span class="input-group-text bg-primary text-light" id="basic-addon1"><i class="fa-regular fa-clipboard"></i></span>
              <input type="text" class="form-control" name="descripcion_bebida" placeholder="Descripción Del Producto" aria-label="Username" aria-describedby="basic-addon1" maxlength="50" style="max-width: 300px;">
            </div>
            <br>
            <div class="input-group mb-3">
              <span class="input-group-text bg-success text-light" id="basic-addon1"><i class="fa-solid fa-dollar-sign"></i></span>
              <input type="text" class="form-control"  name="precio_bebida" placeholder="Precio Unitario" aria-label="Username" aria-describedby="basic-addon1" maxlength="50" style="max-width: 300px;">
            </div>
            <br>
            <div class="input-group mb-3">
              <span class="input-group-text bg-danger" id="basic-addon1"><i class="fa-solid fa-calendar-days text-light"></i></span>
              <input type="date" class="form-control" name="fecha_bebida" aria-label="Username" aria-describedby="basic-addon1" maxlength="50" style="max-width: 300px;">
            </div>
            <br>
            <div class="input-group mb-3">
              <span class="input-group-text bg-warning text-light" id="basic-addon1"><i class="fa-solid fa-boxes-packing"></i></span>
              <input type="number" class="form-control" name="stock_bebida" aria-label="Username" aria-describedby="basic-addon1" maxlength="50" style="max-width: 300px;">
            </div>
            <br>
            <div class="input-group mb-3">
              <span class="input-group-text bg-secondary text-light" id="basic-addon1"><i class="fa-solid fa-industry"></i></span>
              <input type="text" class="form-control" name="proveedor_bebida" placeholder="Proveedor / Empresa" aria-label="Username" aria-describedby="basic-addon1" maxlength="50" style="max-width: 300px;">
            </div>
            <br>
  ...
  <?php while ($row = mysqli_fetch_array($query)): ?>
                    <tr>
                        ...
                        <th><?= $row['descripcion_bebida'] ?></th>
                        <th>S/. <?= $row['precio_bebida'] ?></th>
                        <th><?= $row['fecha_bebida'] ?></th>
                        <th><?= $row['stock_bebida'] ?></th>
                        <th><?= $row['proveedor_bebida'] ?></th>
                        <th><a href="editar_paginabebida.php?id=<?= $row['id'] ?>" class="users-table--edit btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></a></th>
                        <th><a href="borrar_bebida.php?id=<?= $row['id'] ?>" class="users-table--delete btn btn-danger" ><i class="fa-solid fa-trash-can"></i></a></th>
                    </tr>
                <?php endwhile; ?>
  </tbody>
    </table>
